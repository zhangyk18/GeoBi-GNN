...
# ================================================= argparse ===================================================
# python train_dual.py --data_type=Synthetic --sub_size=20000 --wei_type=0 --loss_v_scale=1 --loss_n_scale=1 --gpu= --flag=
# python train_dual.py --data_type=Kinect_v1 --sub_size=20000 --wei_type=0 --loss_v_scale=1 --loss_n_scale=1 --gpu= --flag=
# python train_dual.py --data_type=Kinect_v2 --sub_size=20000 --wei_type=0 --loss_v_scale=1 --loss_n_scale=1 --gpu= --flag=
# python train_dual.py --data_type=Kinect_Fusion --sub_size=20000 --wei_type=0 --loss_v_scale=1 --loss_n_scale=1 --gpu= --flag=
def parse_arguments():
    ...
    parser.add_argument('--loss_n_oriented', action="store_true", help='normal oriented [default: True]')
...
def train(opt):
    ...
    # ------------------------------------------------ 2.data ------------------------------------------------
    print('==='*30)
    dataset1 = dataset.DualDataset('Kinect_v1', 'train', data_list_txt=opt.train_txt, weight_type=opt.wei_type, filter_patch_count=opt.filter_patch_count,
                                   submesh_size=opt.sub_size, transform=dataset.RandomRotate(False))
    dataset2 = dataset.DualDataset('Kinect_v2', 'train', data_list_txt=opt.train_txt, weight_type=opt.wei_type, filter_patch_count=opt.filter_patch_count,
                                   submesh_size=opt.sub_size, transform=dataset.RandomRotate(False))
    train_dataset = dataset.MultiDataset([dataset1, dataset2])
    train_loader = torch.utils.data.DataLoader(train_dataset, shuffle=True, collate_fn=dataset.Collater([]))
    dataset3 = dataset.DualDataset('Kinect_v1', 'test', data_list_txt=opt.test_txt, weight_type=opt.wei_type, submesh_size=opt.sub_size)
    dataset4 = dataset.DualDataset('Kinect_v2', 'test', data_list_txt=opt.test_txt, weight_type=opt.wei_type, submesh_size=opt.sub_size)
    eval_dataset = dataset.MultiDataset([dataset3, dataset4])
    ...
    if opt.U_arch:
        ...
        # net = network.DualGNN_noU(opt.include_facet_initial_feature, opt.adaptive_loss_weight)
        net = None
    ...
    for epoch in range(1, opt.max_epoch):
        ...
        for step, data in enumerate(train_loader):
            ...
            train_loss_f = network.loss_n(norm_p, data[1].y, opt.loss_n, oriented=opt.loss_n_oriented)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(eval_dataset):
                ...
                loss_i_f = network.loss_n(norm_p, data[1].y, opt.loss_n, oriented=opt.loss_n_oriented)
