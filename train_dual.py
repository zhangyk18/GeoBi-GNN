...
# ================================================= argparse ===================================================
# python train_dual.py --data_type=Synthetic --gpu= --flag= --wei_type=3 --include_facet_initial_feature= --learn_res= --AWA=
def parse_arguments():
    ...
    parser.add_argument('--test_txt', type=str, default='test_list.txt', help='Test set list txt list')
    ...
    parser.add_argument('--wei_type', type=int, default=0, help='Edge weight type for Graclus graph pooling, 0:FGC, 1:sigmod, 2:sigmod+propagative')

    parser.add_argument('--lmd_wei', type=float, default=1)
...
def train(opt):
    ...
    # ------------------------------------------------ 2.data ------------------------------------------------
    print('==='*30)
    # train_dataset = dataset.DualDataset(opt.data_type, 'train', data_list_txt=opt.train_txt, filter_patch_count=opt.filter_patch_count,
    ...
    # eval_dataset = dataset.DualDataset(opt.data_type, 'test', data_list_txt=opt.test_txt, submesh_size=opt.sub_size)
    ...
    T = opt.lmd_wei
    avg_loss = np.zeros([opt.max_epoch, 2])
    dual_wei = np.ones([opt.max_epoch, 2]) * 0.5

    for epoch in range(last_epoch, opt.max_epoch):
        print_log = epoch % 10 == 0

        if not (epoch == 0 or epoch == 1):
            w = avg_loss[epoch-1] / avg_loss[epoch-2]
            # w = np.exp(w / T)
            w = w**T
            dual_wei[epoch] = w / w.sum()

        lambda_weight = dual_wei[epoch, 0]
        ...
        # bar = "{l_bar}{bar}{r_bar}"
        desc = "TRAINING - epoch:{:>3} loss:{:.4f} {:.4f} {:.4f} wei:{:.2f}  error:{:.4f} {:.4f}"
        bar = "{desc}  ({n_fmt}/{total_fmt}) [{elapsed}<{remaining}]"
        pbar = tqdm(total=len(train_loader), ncols=90, leave=False, desc=desc.format(epoch, 0, 0, 0, 0, 0, 0), bar_format=bar)
        optimizer.zero_grad()
        for step, data in enumerate(train_loader):
            ...
            train_loss = network.dual_loss(train_loss_v, train_loss_f, v_scale=opt.loss_v_scale, n_scale=opt.loss_n_scale, alpha=lambda_weight)
            train_error_v = network.error_v(vert_p, data[0].y)
            train_error_f = network.error_n(norm_p, data[1].y)

            avg_loss[epoch, 0] += train_loss_v / len(train_loader)
            avg_loss[epoch, 1] += train_loss_f / len(train_loader)
            ...
            if (((step+1) % opt.batch_size) == 0) or (step+1 == len(train_loader)):
                ...
                pbar.desc = desc.format(epoch, train_loss_v, train_loss_f, train_loss, lambda_weight, train_error_v, train_error_f)
        ...
        str_log = F"Epoch {epoch:>3}: {str(span).split('.')[0]:>8}  loss:{eval_loss_v:.4f} {eval_loss_f:.4f} wei:{lambda_weight:.2f} | "
